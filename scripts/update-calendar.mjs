...
const sources = [
  {
    ...
    color: '#45dbe6',
    textColor: '#062f33',
    url: 'https://southsimcoeminorhockey.ca/webcal.ashx?IDs=1149'
  },
  {
    team: 'Storm U12 A',
    color: '#004e8a',
    textColor: '#ffffff',
    url: 'https://southsimcoeminorhockey.ca/webcal.ashx?IDs=1154&O=0'
  }
];
...
for (const source of sources) {
  const res = await fetch(source.url, {
    headers: {
      'User-Agent': 'Storm-Friends-Shared-Calendar/1.0'
    }
  });

  if (!res.ok) {
    throw new Error(
      `Failed ${source.team}: HTTP ${res.status}`
    );
  }
  ...
  for (
    const vevent of comp.getAllSubcomponents('vevent')
  ) {
    ...
    /*
      Any event containing " - Game " is treated
      as a game. Everything else is treated as
      practice/training.
    */
    const type =
      title.includes(' - Game ')
